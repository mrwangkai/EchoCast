...
- [ ] T63 CarPlay — Add Note button crash or silent failure when tapped Priority: P1 | LOE: M Description:
Untested in Build 8: tapping the "Add Note" (square.and.pencil) button on the CarPlay Now Playing screen triggers AddNoteIntent().perform() with openAppWhenRun: true. This is expected to bring EchoCast to the foreground on the phone and open the note capture sheet at the current playback time. Verify: (1) the intent fires without crashing, (2) the phone app opens to the correct note capture sheet, (3) playback continues on CarPlay while the note sheet is open on the phone. If the app crashes or the sheet doesn't appear, investigate AddNoteIntent in the App Intents implementation and confirm openAppWhenRun is still set to true.
