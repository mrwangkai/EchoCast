...
- [ ] T59 #CarPlay (P1): Album artwork not loading on Now Playing screen — Observed in Build 8: the CarPlay Now Playing screen shows a generic music note placeholder instead of the episode's podcast artwork. The CPNowPlayingImageButton or MPNowPlayingInfoCenter metadata is not being populated with artwork. Investigate CarPlayNowPlayingController — confirm MPMediaItemArtwork is being created from the episode's artwork URL and set on MPNowPlayingInfoCenter.default().nowPlayingInfo. Artwork fetch may need to happen asynchronously before setting. Do not modify phone-side player UI. (LOE: S)
...
- [ ] T61 #CarPlay (P2): Tapping a podcast in "My Podcasts" tab does nothing — Observed in Build 8: the My Podcasts tab correctly lists followed podcasts, but tapping any row has no effect. Expected behavior: tapping a podcast should push a new CPListTemplate showing that podcast's episode list (fetched via the existing RSS feed mechanism). Each episode row should be tappable and trigger playback via GlobalPlayerManager. The episode list template should have a back button (CarPlay handles this natively via CPNavigationController — just push the new template). This is a new navigation flow: CPListTemplate (My Podcasts) → push CPListTemplate (Episode List) → tap episode → play. Reuse the same episode playback path being fixed in T58. Do not build a custom back button — use CPNavigationController.push(_:animated:) which provides it automatically. (LOE: M)
